...
Evento 1: Um IP tenta se autenticar via ssh múltiplas vezes em um período de tempo espaçado, para não ser detectado por regras de brute force. Como pode ser observado no dashboard abaixo, mais especificamente no segundo painél da primeira coluna (IPs com mais erros de autenticação SSH), os top 5 IPs com mais erros serão mostrados ali, independentemente se foi um brute force agressivo ou um espaçado com o objetivo de bypassar as regras de detecção. E no segundo painél da segunda coluna (Tabela de IPs com erro de autenticação via SSH (origem e destino)) podemos observar a origem do IP que está tentando acessar e o usuário de destino.

![dashboard](/img/dashboard1.png)

Evento 2: O IP consegue um login bem sucedido via ssh sem gerar alertas, mas é facilmente notado no dashboard que houve um login bem sucedido vindo do mesmo IP que teve mais erros de autenticação (192.168.1.16), que pode ser observado no primeiro painél da primeira coluna (Timeline de autenticações bem sucedidas ssh) e no primeiro painél da segunda coluna (Tabela de IPs que logaram via SSH (origem e destino)). Ligando os pontos, até então, temos um IP que pode ser externo e suspeito ou no caso desse laboratório, um IP interno que cometeu ações suspeitas e realizou um login bem sucedido. 

![dashboard](/img/dashboard1.png)

Evento 3: Um comando sensível é digitado logo após a autenticação como pode ser visualizado na timeline de comandos do painél 3 da primeira coluna (Timeline comandos sensíveis digitados por agentes) e no painél 3 da segunda coluna (Tabela de comandos sensíveis digitados por agentes) é mostrado qual comando foi digitado e qual usuário digitou. Nessa etapa fica claro que o atacante após realizar o login, fez um reconhecimento na rede e tentou adicionar um novo usuário para manter uma persistência, o que classifica como um sinal vermelho de pós-exploração. 
